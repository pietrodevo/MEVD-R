...
def jocker(
    ...
    lengths=3,
    coefficients=None,
    distributions=["gev", "mev", "smev", "cmev"],
    parameters={},
    outputs=2,
    dictionaries=None,
    check=None,
    ...
    if skip and (
        ...
        try:
            ...
            for combination, idx, length in utils.iterables(
                ...
                if any(i in ["mev", "cmev"] for i in distributions):
                    # computing moments
                    ...
                    array_moments = region.projector(
                        ...
                        duration,
                        size=["λ"],
                        labels=["L%s" % (i) for i in range(1, outputs + 1)],
                        clusters=idx_calibration,
                        infer=True,
                        blocks=True,
                    )

                if "cmev" in distributions and isinstance(parameters, dict):
                    # computing parameters
                    dataframe_parameters = {
                        i: region.blockranger(
                            None,
                            data_calibration,
                            idx_calibration,
                            duration,
                            ...
                            outputs=[
                                "X%s" % (i) for i in range(1, outputs + 1)
                            ],
                            keywords=j,
                            filename=None,
                            level=1,
                    ...
                    array_parameters = {
                        ...
                        for i, j in dataframe_parameters.items()
                    }
                else:
                    parameters = None
                        ...
                        quantiles.append(
                            cross.function(
                                ...
                                *(
                                    ...
                                    else array_parameters[i]
                                ),
                                indexer_calibration,
                                parameters=parameters,
                                distribution=parameters[i]["distribution"],
                                coefficients=array_coefficients,
                                v=annual.min(),
                                τ=τ_data,
                                κ=κ_data,
                                key=(
                ...
                if check is None:
                    ...
                    quantiles = pandas.concat([i[0] for i in quantiles])
                elif drop == "variable":
                    quantiles = pandas.concat(
                        [i[0].loc[(i[1], slice(None)), :] for i in quantiles]
                    )
                elif drop == "consistent":
                    quantiles = pandas.concat([i[0] for i in quantiles]).loc[
                        (
                            functools.reduce(
                                numpy.intersect1d,
                                [i[1] for i in outputs],
                                slice(None),
                            ),
                        ),
                        :,
